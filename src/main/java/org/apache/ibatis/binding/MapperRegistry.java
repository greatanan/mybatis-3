 ...
 * @author Lasse Voss
 *
 * //mynote: MapperRegisy 是 Mapper 接口及其对应的代理对象工厂的注册中心。 Configuration 是
 *           MyBatis 全局性的配置对象，在 MyBatis 初始化的过程中，所有配置信息会被解析成相应的对
...
public class MapperRegistry {
  ...
  /**
   *    在 MyBatis 初始化过程中会读取映射配置文件以及 Mapper 接 口中 的注解信息 ，并调用
   *     MapperRegistry.addMapperO方法填充 MMapperRegistry.knownMappers 集合 ， 该集合的 key 是
   *     Mapper 接口对应的 Class 对象， value 为 MapperProxyFactory 工厂对象，可以为 Mapper 接口创
   *    建代理对象
   * @param type
   * @param <T>
   */
